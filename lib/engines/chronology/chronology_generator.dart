...
class ChronologyItem {
  ...
  /// Parses and checks an item. Beyond the engine rules, every event's year
  /// must appear in at least one cited excerpt, so no date ships unsourced.
  static ChronologyItem parse(Map<String, dynamic> item) {
    final payload = item['payload'];
    final reveal = item['reveal'];
    if (payload is! Map || reveal is! Map) throw const FormatException('Chronology item needs "payload" and "reveal"');
    final puzzle = ChronologyPuzzle.parse(Map<String, dynamic>.from(payload), Map<String, dynamic>.from(reveal));
    final rawTopic = item['topic'];
    final topic = rawTopic is String && rawTopic.trim().isNotEmpty ? rawTopic : puzzle.title;
    if (topic == null) throw const FormatException('Chronology item needs a "topic"');
    final rawSources = item['sources'];
    if (rawSources is! List || rawSources.isEmpty) throw const FormatException('Chronology item needs "sources"');
    final sources = [for (final s in rawSources) SourceRef.fromJson(Map<String, dynamic>.from(s as Map))];
    for (final id in puzzle.order) {
      final year = puzzle.years[id]!.abs();
      final pattern = RegExp('(?<!\\d)$year(?!\\d)');
      if (!sources.any((s) => pattern.hasMatch(s.excerpt))) {
        throw FormatException('Chronology event "$id": no source excerpt contains the year $year');
      }
    }
...
///
/// The pick for a date is `fnv1a('chronology-<date>') % count`, stepped
...
class ChronologyGenerator {
  ChronologyGenerator({this.reserveDir = defaultReserveDir}) {
    _load();
  }

  static const String slug = 'chronology';
  static const String defaultReserveDir = 'content_src/editorial/chronology';
  ...
  final List<ChronologyItem> _items = [];
  ...
  List<ChronologyItem> get items => List.unmodifiable(_items);
  ...
  void _load() {
    ...
    for (final path in paths) {
      ...
      try {
        ...
        _items.add(ChronologyItem.parse(json));
        _files.add(name);
  ...
  void _ensureReady() {
    if (problems.isNotEmpty) throw StateError('Chronology reserve has problems: ${problems.join('; ')}');
    if (_items.length < minReserve) {
      throw StateError('Chronology reserve has ${_items.length} puzzles; at least $minReserve are needed');
    }
  }
  ...
  PuzzleRecord fromTemplate(DateTime date, Map<String, dynamic> item, {String? storyId}) =>
      _wrap(date, ChronologyItem.parse(item), storyId: storyId);

  PuzzleRecord _wrap(DateTime date, ChronologyItem item, {String? storyId}) => PuzzleRecord(
        id: PuzzleId(game: GameKind.chronology, date: DateTime.utc(date.year, date.month, date.day)),
        locale: 'en-GB',
        contentVersion: 1,
        scoringVersion: 1,

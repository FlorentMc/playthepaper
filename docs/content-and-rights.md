...
| News editions (nightly routine) | stories chosen from keyless public feeds: Wikipedia Current Events portal (CC BY-SA 4.0), NASA science.nasa.gov and APOD (US government work, public domain unless credited otherwise), Smithsonian Magazine, ScienceDaily and Quanta Magazine RSS feeds (publisher copyright) | quiz text, summaries, headlines and clues ours; excerpts are short verbatim quotations (at most 300 characters, at most three per source) | publisher named and article linked in the app; excerpts shown only in reveals with the source; no article text republished; sources that refuse automated fetching are not used (see infra/routine/PROMPT.md SOURCES) | The Quiz, seeds for Daily Word, Letters, Mini Crossword |

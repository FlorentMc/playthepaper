...
import 'package:playthepaper/engines/linked/linked_puzzle.dart';

/// Writes the Linked Clues puzzle files for a date range from the evergreen
/// reserve in content_src/editorial/linked.
///
///   dart run tool/gen_linked.dart --from 2026-09-10 --to 2026-10-31 --out content/puzzles [--force]
///
/// Every reserve file is parsed first and each failure is reported. The pick
/// for a date is a stable FNV-1a hash of `linked-<date>`, avoiding any
...
void main(List<String> args) {
  ...
  if (options == null) {
    stderr.writeln('usage: dart run tool/gen_linked.dart --from YYYY-MM-DD --to YYYY-MM-DD --out <dir> [--force]');
    exitCode = 64;
    return;
  }
...
void _verify(File file, PuzzleRecord expected) {
  ...
  final generated = LinkedPuzzle.parse(expected.payload, expected.reveal);

...
import 'package:playthepaper/engines/groups/groups_puzzle.dart';

/// Writes the Groups puzzle files for a date range from the evergreen
/// reserve in content_src/editorial/groups.
///
///   dart run tool/gen_groups.dart --from 2026-09-10 --to 2026-10-31 --out content/puzzles [--force]
///
/// Every reserve file is parsed first and each failure is reported. The pick
/// for a date is a stable FNV-1a hash of `groups-<date>`, avoiding any file
/// used in the previous thirty days; the board is laid out from the same
/// seed. Existing files are kept unless `--force`. Every written file is
/// re-read and re-parsed.
void main(List<String> args) {
  final options = _parse(args);
  if (options == null) {
    stderr.writeln('usage: dart run tool/gen_groups.dart --from YYYY-MM-DD --to YYYY-MM-DD --out <dir> [--force]');
    exitCode = 64;
    return;
  }
  ...
  for (var date = options.from; !date.isAfter(options.to); date = date.add(const Duration(days: 1))) {
    ...
    _verify(file, record);
    final pick = generator.pickFor(date);
    stdout.writeln('$dateText  ${generator.files[pick]} · ${generator.items[pick].topic} · ${clock.elapsedMilliseconds}ms');
    written++;
  }
...
void _verify(File file, PuzzleRecord expected) {
  ...
  final generated = GroupsPuzzle.parse(expected.payload, expected.reveal);

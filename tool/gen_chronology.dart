...
import 'package:playthepaper/engines/chronology/chronology_puzzle.dart';

/// Writes the Before & After puzzle files for a date range from the
/// evergreen reserve in content_src/editorial/chronology.
///
///   dart run tool/gen_chronology.dart --from 2026-09-10 --to 2026-10-31 --out content/puzzles [--force]
///
/// Every reserve file is parsed first and each failure is reported. The pick
/// for a date is a stable FNV-1a hash of `chronology-<date>`, stepped past the
/// recent picks so a puzzle does not come round again for at least thirty
/// days. Existing files are kept unless `--force` is given. Every written
/// file is re-read and re-parsed.
void main(List<String> args) {
  final options = _parse(args);
  if (options == null) {
    stderr.writeln('usage: dart run tool/gen_chronology.dart --from YYYY-MM-DD --to YYYY-MM-DD --out <dir> [--force]');
    exitCode = 64;
    return;
  }
  ...
  for (var date = options.from; !date.isAfter(options.to); date = date.add(const Duration(days: 1))) {
    ...
    final topic = generator.items[generator.pickFor(date)].topic;
    ...
    _verify(file, record);
    stdout.writeln('$dateText  ${generator.files[generator.pickFor(date)]} · $topic · ${clock.elapsedMilliseconds}ms');
    written++;
  }
...
void _verify(File file, PuzzleRecord expected) {
  ...
  final generated = ChronologyPuzzle.parse(expected.payload, expected.reveal);

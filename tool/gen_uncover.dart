...
import 'package:playthepaper/engines/uncover/uncover_generator.dart';
import 'package:playthepaper/engines/uncover/uncover_puzzle.dart';

/// Writes the Uncover puzzle files for a date range from the evergreen
/// reserve in content_src/editorial/uncover.
///
///   dart run tool/gen_uncover.dart --from 2026-09-10 --to 2026-10-31 --out content/puzzles [--force]
///
/// Every reserve file is parsed first and each failure is reported. The pick
/// for a date is a stable FNV-1a hash of `uncover-<date>`, moved on while it
/// repeats a pick of the previous thirty days. Existing files are kept unless
/// `--force` is given. Every written file is read back and re-parsed.
void main(List<String> args) {
  final options = _parse(args);
  if (options == null) {
    stderr.writeln('usage: dart run tool/gen_uncover.dart --from YYYY-MM-DD --to YYYY-MM-DD --out <dir> [--force]');
    exitCode = 64;
    return;
  }
  final generator = UncoverGenerator();
  final problems = generator.checkReserve();
  for (final entry in problems.entries) {
    stderr.writeln('reserve: ${entry.key}: ${entry.value}');
  }
  if (problems.isNotEmpty) {
    stderr.writeln('${problems.length} reserve file(s) failed to parse');
    exitCode = 1;
    return;
  }
  stdout.writeln('Reserve: ${generator.reserve.length} puzzles in ${generator.reserveDir}');
  ...
  for (var date = options.from; !date.isAfter(options.to); date = date.add(const Duration(days: 1))) {
    ...
    final item = generator.itemFor(date);
    ...
    _verify(file, record);
    stdout.writeln('$dateText  ${item.file} · ${item.topic} · ${clock.elapsedMilliseconds}ms');
    written++;
  }
...
void _verify(File file, PuzzleRecord expected) {
  ...
  final generated = UncoverPuzzle.parse(expected.payload, expected.reveal);
  if (puzzle.subject != generated.subject ||
      puzzle.text != generated.text ||
      puzzle.maskedText != generated.maskedText ||
      record.sources.length != expected.sources.length) {
